...
function generateRandomColor() {
	function _(x) {
		return Math.floor(Math.random() * (255 - x + 1)) + x;
	}
	const start = 200;
	var r = _(start);
	var g = _(start);
...
function setRandomGradientBackground(tag) {
	var gradientColor1 = generateRandomColor();
	var gradientColor2 = generateRandomColor();
	var gradientColor3 = generateRandomColor();
	var angle = Math.floor(Math.random() * 361);
	var renderBody = document.querySelector(tag);
	renderBody.style.background = `linear-gradient(${angle}deg, ${gradientColor1}, ${gradientColor2}, ${gradientColor3})`;
}
...
document.addEventListener("DOMContentLoaded", () => {
	setRandomGradientBackground("body");
